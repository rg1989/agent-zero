...
<context>
State and phase progress are gathered with targeted reads.
</context>
...
Parse the phase directory name from the output.
...
The handoff file format (`.continue-here.md`):
...
Use `code_execution_tool` to get the current timestamp for the `last_updated` field:
```bash
...
<step name="commit">
Stage and commit using `code_execution_tool`:
```bash
...
To resume: use the gsd-resume-work skill
```
</step>
