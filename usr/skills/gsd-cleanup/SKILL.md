...
<objective>
Archive phase directories from completed milestones into `.planning/milestones/v{X.Y}-phases/`. Identifies which phases belong to each completed milestone, shows a dry-run summary, and moves directories on confirmation.
...
Read `.planning/MILESTONES.md` to identify completed milestones and their versions using `code_execution_tool`:
...
Use `input` to ask: "Proceed with archiving?" with options: "Yes -- archive listed phases" | "Cancel"
...
For each milestone, move phase directories using `code_execution_tool`:
...
<step name="commit">
Commit the changes using `code_execution_tool`:
