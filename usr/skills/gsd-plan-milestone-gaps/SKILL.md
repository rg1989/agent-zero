...
<objective>
Create all phases necessary to close gaps identified by the `gsd-audit-milestone` skill.
...
Find the most recent audit file in `.planning/`:
...
```
No audit gaps found. Run the `gsd-audit-milestone` skill first.
```
...
Find the highest existing phase by reading ROADMAP.md and scanning all phase entries for the highest phase number.
...
Verify: count `Pending` entries in REQUIREMENTS.md to confirm updates applied.

## 8. Create Phase Directories

For each new gap closure phase, create its directory:
`.planning/phases/{NN}-{name}`

## 9. Commit Roadmap and Requirements Update

Commit: `docs(roadmap): add gap closure phases {N}-{M}` — include ROADMAP.md and REQUIREMENTS.md.
...
**Also available:**
- Use `gsd-execute-phase` if plans already exist
- Review `.planning/ROADMAP.md` to see updated roadmap

**After all gap phases complete:**

Use the `gsd-audit-milestone` skill — re-audit to verify gaps closed
Use the `gsd-complete-milestone` skill — archive when audit passes
```
...
**Flow gap → Tasks:** Flow gaps often overlap with requirement and integration gaps. Group them into the same phase where applicable.
...
- [ ] User knows to use `gsd-plan-phase` next
</success_criteria>
