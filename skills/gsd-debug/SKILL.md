...
```bash
INIT=$(node /root/.codex/get-shit-done/bin/gsd-tools.cjs state load)
```

Extract `commit_docs` from init JSON. Resolve debugger model:
```bash
DEBUGGER_MODEL=$(node /root/.codex/get-shit-done/bin/gsd-tools.cjs resolve-model gsd-debugger --raw)
```
