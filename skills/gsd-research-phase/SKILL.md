...
```bash
INIT=$(node /root/.codex/get-shit-done/bin/gsd-tools.cjs init phase-op "{{GSD_ARGS}}")
```
...
```bash
RESEARCHER_MODEL=$(node /root/.codex/get-shit-done/bin/gsd-tools.cjs resolve-model gsd-phase-researcher --raw)
```

## 1. Validate Phase

```bash
PHASE_INFO=$(node /root/.codex/get-shit-done/bin/gsd-tools.cjs roadmap get-phase "${phase_number}")
```
...
```
Task(
  prompt="First, read /root/.codex/agents/gsd-phase-researcher.md for your role and instructions.\n\n" + filled_prompt,
